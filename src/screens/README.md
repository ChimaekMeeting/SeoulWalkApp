...
`HomeScreen`(홈 탭)은 `src/bottomsheets/ChatBottomSheet.tsx`로 감싼 바텀시트 안에 `ChatConversation`이 임베드되는 구조입니다. 키보드 대응은 Android의 `adjustResize`와 iOS 앱 셸의 `KeyboardAvoidingView`가 현재 레이아웃 자체를 줄이는 방식으로 통일하고, `HomeScreen`은 `onLayout`으로 그 실제 높이와 입력창 높이를 측정합니다. 키보드 높이를 별도 오프셋으로 다시 더하지 않습니다. 자세한 내용은 [`src/README.md`의 "5. bottomsheets/"](../README.md)와 [`src/components/README.md`의 "2. chat/"](../components/README.md) 참고.
...
WalkCompleteScreen (6d)
   ...
   │ "평가 없이 홈으로" / 안드로이드 뒤로가기 → onExitToHome
   ▼
...
- **`WalkCompleteScreen` (6d)**: 완주/중간 종료 구분 없이 항상 "산책 완료! 🎉 축하합니다!" — "걸은 만큼 인정"이 방침이라 종료 방식을 화면에서 안 나눈다. 거리(`routeProgressKm`)/시간/걸음 수 요약, 즐겨찾기 토글. "산책로 평가하기"는 `stage: 'rating'`으로 이동하고, "평가 없이 홈으로"나 안드로이드 뒤로가기는 바로 `onExitToHome`을 호출한다. 평가는 선택사항이다. `WalkFlow`는 스냅샷의 `endReason`으로 세션 리셋 사유만 `completed`/`ended_early`로 구분한다(챗봇 세션 처리용, UI엔 안 보임).
- **`WalkRatingScreen` (6e)**: 방금 걸은 산책로가 얼마나 마음에 들었는지 별점 3개(안전·편안함·전체 만족도, 각 1~5)로 확인한다. 재사용 컴포넌트 `src/components/StarRating.tsx`. 3개를 모두 매겨야 "완료"가 활성화되고, 누르면 `POST /api/user/routes/{id}/feedback` 저장이 성공한 뒤 `onExitToHome`으로 이동한다. 후보 대조가 부족한 `insufficient_candidates`는 별점 저장 성공으로 처리한다. 평가 화면에서도 "이번에는 건너뛰기"로 홈에 갈 수 있고, 통신 실패 시 재시도할 수 있다.
